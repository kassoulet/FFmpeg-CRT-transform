...
fn blur_v(img: &ImgF32, k: &[f32]) -> ImgF32 {
    ...
    // Optimized vertical blur: process rows in parallel, but for each output row,
    // we accumulate contributions from source rows. This keeps memory access
    // linear within each row, significantly improving cache locality compared
    // to jumping between rows for every pixel.
    out.data.par_chunks_exact_mut(row_stride)
        .enumerate()
        .for_each(|(y, row_out)| {
            for (j, &w) in k.iter().enumerate() {
